...
function Monte_Carlo_Heating(its,N,mc_steps,T_start,T_fin)
    ...
    for i=1:its
        ...
        while T <= T_fin
            ...
            T = T + dT;
            mc_steps_eq = 10^5;
            [config_T_eq,E_T,mean_dR(end+1)]=MC_Routine('Coulomb',N,mc_steps_eq,d_max,config_T,T,1);
    
            %
            mc_steps_T=10^3;
            [config_T,E_T,mean_dR(end+1)]=MC_Routine(int_pot,N,mc_steps_T,d_max,config_T_eq,T,1);
            for k=1:N
                r_k=[];
